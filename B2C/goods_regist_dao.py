...
class GoodsRegistDao:
    ...
    def insertGoods(self, out_item_no, category_code, item_no, item_name, gd_html, maker_no, expiration_date,
                    price, default_image, large_image, small_image, auto_term_duration, auto_use_term_duration,
                    use_information, help_desk_telno,
                    apply_place, apply_place_url, apply_place_telephone, display_date, stock_qty, user_id,
                    shippinggroupcode=None):
        ...
        if (len(goods) > 0):
            db.executeQuery(self.query_update_goods, datetime.datetime.now(), out_item_no, category_code, item_no,
                            item_name, gd_html, maker_no,
                            expiration_date, price, default_image, large_image, small_image, auto_term_duration,
                            auto_use_term_duration, use_information,
                            help_desk_telno, apply_place, apply_place_url, apply_place_telephone, display_date,
                            stock_qty, user_id, shippinggroupcode, item_no)
        else:
            db.executeQuery(self.query_insert_goods, datetime.datetime.now(), datetime.datetime.now(), out_item_no,
                            category_code, item_no, item_name, gd_html, maker_no,
                            expiration_date, price, default_image, large_image, small_image, auto_term_duration,
                            auto_use_term_duration, use_information,
                            help_desk_telno, apply_place, apply_place_url, apply_place_telephone, display_date,
